...
def load_data():  
    ...
    filename_user_movie = 'data/u.changed'  
...
def getRecommendations(user, user_movie, movies, userSim, N):  
    ...
    # top-10 pred  
    pred0 = sorted(pred.items(), key=itemgetter(1), reverse=True)
    pred = [item for item in pred0 if item[1] >average_u_rate]
    return pred    
  
  
if __name__ == "__main__":  
    
    actual_file = open('data/actual_c.txt','w')
    pred_file = open('data/pred_c.txt','w')
    ...
    i = 0
    for users in user_movie.keys():
        print i
        i+=1
        average_u_rate = average_rating(users)
        actual_file.write(unicode(users)+' ')
        for key in user_movie[users].keys():
            if user_movie[users][key] > average_u_rate:
                actual_file.write(unicode(key)+' ')
        actual_file.write('\n')
    # Recommend  
        pred = getRecommendations(users, user_movie, movies, userSim, 20)
        
        pred_file.write(unicode(users)+' ')
        for item in pred:
            pred_file.write(unicode(item[0])+' ')
        pred_file.write('\n')

  
    #print 'Recommendations(%s):' %(len(pred))
    # display recommend result (top-10 results)  
    # for i, rating in pred:  
    #     print 'film: %s,  rating: %s' % (movies[i], rating)
        #print '%s,  rating: %s' %(i,rating)  